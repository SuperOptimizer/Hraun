...
def load_cropped_tiff_slices(tiff_directory, start_slice, end_slice, crop_start, crop_end):
    tiff_files = [f for f in os.listdir(tiff_directory) if f.lower().endswith('.tif')]
    max_number = max([int(f.split('.')[0]) for f in tiff_files])
    num_digits = len(str(max_number))

    slices_data = []
    for slice_index in range(start_slice, end_slice):
        tiff_filename = f"{slice_index:0{num_digits}d}.tif"
...
class VolMan:
    def __init__(self, cachedir='scroll_cache'):
    ...
    def chunk(self, scroll, source, idnum, yoff=None, xoff=None, zoff=None, ysize=None, xsize=None, zsize=None, ychunk=None,xchunk=None,zchunk=None):
        ...
        if source == 'volume_grids':
            ...
            dl_path = os.path.join(self.cachedir, self._make_url(scroll, source, idnum)).replace('http://', '')
            for idx in range(start, end):
                ...
                if os.path.exists(local_file):
                    print(f"skipping downloading {local_file}")
                    continue
                ...
                if response.status_code == 200:
                    ...
                    tiff_data = tifffile.imread(local_file)
                    if tiff_data.dtype == np.uint8:
                        continue
                    tiff_data //= 256
                    tiff_data = tiff_data.astype(np.uint8)
                    tifffile.imwrite(local_file, tiff_data, compression='zlib')
            ...
            data = load_cropped_tiff_slices(dl_path, start, end, crop_start, crop_end)
