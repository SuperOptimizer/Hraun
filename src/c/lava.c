...
static s32 get_index(s32 z, s32 y, s32 x)
{
    return z / 2 * 4 + y / 2 * 2 + x / 2;
}

static u8 get_bit_position(s32 z, s32 y, s32 x)
{
    return z % 2 * 4 + y % 2 * 2 + x % 2;
}
...
void bitmask3d_set(bitmask3d* mask, s32 z, s32 y, s32 x, bool value)
{
    ...
    if (value) {
        mask->data[byte_index] |= (1 << bit_position);
    } else {
        mask->data[byte_index] &= ~(1 << bit_position);
    }
}
...
void bitmask3d_print(bitmask3d* mask)
{
    for (s32 z = 0; z < mask->depth; z++) {
        ...
        for (s32 y = 0; y < mask->height; y++) {
            for (s32 x = 0; x < mask->width; x++) {
                printf("%d", bitmask3d_get(mask, z, y, x) ? 1 : 0);
            }
            printf("\n");
        }
        printf("\n");
